...
bool EmulateTag::emulate(const uint16_t tgInitAsTargetTimeout){
  ...
  if(1 != HFC.tgInitAsTarget(command,sizeof(command), tgInitAsTargetTimeout)){
  ...
  while(runLoop){
    status = HFC.tgGetData(rwbuf, sizeof(rwbuf));
    if(status < 0){
    ...
    status = HFC.tgSetData(rwbuf, sendlen);
    if(status < 0){
