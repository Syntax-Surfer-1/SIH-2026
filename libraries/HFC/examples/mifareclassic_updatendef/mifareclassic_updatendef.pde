...
/*!
    ...
    To enable debug message, define DEBUG in HFC/HFC_debug.h
*/
/**************************************************************************/
  ...
  HFC_SPI HFCspi(SPI, 10);
  HFC nfc(HFCspi);
#elif 1
  #include <HFC_HSU.h>
  #include <HFC.h>
      
  HFC_HSU HFChsu(Serial1);
  HFC nfc(HFChsu);
#else 
  #include <Wire.h>
  #include <HFC_I2C.h>
...
void setup(void) {
  Serial.begin(115200);
  Serial.println("Looking for HFC...");
...
void loop(void) {
  uint8_t success;                          // Flag to check if there was an error with the HFC
  ...
  success = nfc.readPassiveTargetID(HFC_MIFARE_ISO14443A, uid, &uidLength);
